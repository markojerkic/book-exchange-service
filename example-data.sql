...
insert into book (isbn, title, book_author_id)
select '978-1-60309-025-4', 'Na Drini ćuprija', 1
where not exists(select * from book where id = 1);
insert into book (isbn, title, book_author_id)
select '978-1-60309-025-3', 'Čuvaj se senjske ruke', 2
where not exists(select * from book where id = 2);
insert into book (isbn, title, book_author_id)
select '978-1-60309-025-2', 'Prosjaci i sinovi', 3
where not exists(select * from book where id = 3);
insert into book (isbn, title, book_author_id)
select '978-1-60309-025-1', 'Harry Potter', 4
